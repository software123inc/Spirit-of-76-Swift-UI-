...
struct FavoritesList: View {
    ...
    var body: some View {
        List {
            if favs.count > 0 {
                if let persons = (favs.filter{ $0.isKind(of: Person.self )} as? [Person]) {
                ...
                if let events = (favs.filter{ $0.isKind(of: Event.self )} as? [Event]) {
                ...
                if let topics = (favs.filter{ $0.isKind(of: Topic.self )} as? [Topic]) {
